...
def run():
    remove_creat_dir(ns_trips_economic_profits_dir)
    csv_files = get_all_files(nightsafari_trips_dir, ns_trip_prefix, '.csv')
    #
    init_multiprocessor()
    count_num_jobs = 0
    for fn in csv_files:
...
def process_file(fn):
    ...
    with open('%s/%s' % (nightsafari_trips_dir, fn), 'rb') as r_csvfile:
        reader = csv.reader(r_csvfile)
        headers = reader.next()
        hid = {h : i for i, h in enumerate(headers)}
        with open('%s/%s%s.csv' % (ns_trips_economic_profits_dir, ns_trips_ecoprof_prefix, yymm), 'wt') as w_csvfile:
            writer = csv.writer(w_csvfile) 
            new_headers = ['did',
                           'start-time', 'yy', 'mm', 'dd', 'hh',
                           'duration', 'fare',
                           'ns-trip-mode', 'ns-queue-time',
                           'economic-profit']
            ...
            for row in reader:
                ...
                eco_profit = fare - ns_out_productivities[k] * (qt + dur)

                    ...
                    for com_did in com_drivers:
                        for tf in range(PM2, PM11 + 1):
                            for (zi, zj) in zones.iterkeys():
                                if zi < 0 or zj < 0:
                                    continue
                                day_tf_zone_driver[handling_day, tf, zi, zj, com_did] = \
                                    O_APPEAR if tf_zone_driver.has_key((tf, zi, zj, com_did)) else X_APPEAR
                    handling_day = day
                    tf_zone_driver = {}
                did = int(row[hid['did']])
                tf = int(row[hid['timeFrame']])
                zi, zj = int(row[hid['zi']]), int(row[hid['zj']])
                tf_zone_driver[tf, zi, zj, did] = O_APPEAR
                #
                row = reader.next()
        save_pickle_file(tf_zone_drivers_fpath, day_tf_zone_driver)

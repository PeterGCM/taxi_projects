...
def gen_summary1001():
    intellect1001_fpath = '%s/%s%s.csv' % (statisticsAllDrivers_ap_dpath, statisticsAllDriversIntellect_ap_prefix, '1001')
    with open(intellect1001_fpath, 'wt') as w_csvfile:
    ...
    #
    driverIntellect1001 = load_pickle_file('%s/%s%s.pkl' % (statisticsAllDrivers_ap_dpath, statisticsAllDriversIntellect_ap_prefix, '1001'))
    df = pd.read_csv('%s/Filtered-%s%s.csv' % (statisticsAllDrivers_ap_dpath, statisticsAllDriversDay_ap_prefix, '2010'))
    m_df = df[(df['month'] == 1)].copy(deep=True)
    agg_df = m_df.groupby(['driverID']).sum().reset_index()
    candi_drop_cn = []
    ...
    for did, (_, coef) in driverIntellect1001.iteritems():
        if coef == 'X':
            continue
        if did not in allDrivers:
            continue
        with open(intellect1001_fpath, 'a') as w_csvfile:
            writer = csv.writer(w_csvfile, lineterminator='\n')
            new_row = [did, coef]
